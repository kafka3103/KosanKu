-- ============================================================
-- KosanKu — Migration 003: PostgreSQL Functions & Scheduled Jobs
-- Supabase PostgreSQL + pg_cron
-- Deskripsi: Functions yang dipanggil Edge Function dan
--            setup scheduled job via pg_cron
-- ============================================================

-- ============================================================
-- FUNCTION: Generate nomor invoice otomatis
-- Format: INV-YYYY-MM-{sequence 4 digit}
-- ============================================================
...
-- ============================================================
-- FUNCTION: Generate tagihan bulanan untuk semua kontrak aktif
-- Dipanggil oleh Edge Function 'generate-monthly-billing'
-- via pg_cron setiap hari tengah malam
-- ============================================================
...
BEGIN
  ...
  -- Hitung periode billing saat ini
...
-- ============================================================
-- FUNCTION: Expire pengajuan sewa yang sudah melewati batas waktu
-- Dipanggil oleh Edge Function 'expire-rental-requests'
-- Batas: 3 hari kerja (Senin-Jumat) setelah created_at
-- ============================================================
...
-- ============================================================
-- FUNCTION: Hitung expires_at untuk rental request baru
-- (3 hari kerja = skip Sabtu & Minggu)
-- ============================================================
...
BEGIN
  ...
  -- Set ke akhir hari kerja tersebut (23:59:59)
...
-- ============================================================
-- FUNCTION: Dapatkan summary keuangan Owner per properti
-- Digunakan di Dashboard & Laporan Keuangan (PROBIS-07)
-- ============================================================
...
BEGIN
  RETURN QUERY
  SELECT
    -- Total pendapatan (payment berhasil dalam rentang waktu)
...
-- ============================================================
-- FUNCTION: Auto-end contract yang sudah melewati end_date
-- ============================================================
...
-- ============================================================
-- FUNCTION: Auto-update invoice status ke 'overdue'
-- ============================================================
...
-- (Uncomment setelah pg_cron diaktifkan di Supabase project)
-- ============================================================

-- Cek & expire pengajuan sewa kadaluarsa — setiap jam
SELECT cron.schedule(
  'expire-rental-requests-hourly',
  '0 * * * *',
  $$ SELECT expire_overdue_rental_requests(); $$
);

-- Tandai invoice overdue — setiap tengah malam
SELECT cron.schedule(
...
-- Auto-end kontrak yang sudah habis masa — setiap tengah malam
SELECT cron.schedule(
  'auto-end-expired-contracts-daily',
  '5 0 * * *',
  $$ SELECT auto_end_expired_contracts(); $$
);

-- Generate tagihan bulanan — dijalankan setiap hari
-- Edge Function yang akan mengecek per properti mana yang harus generate
SELECT cron.schedule(
  'trigger-billing-generation-daily',
  '10 0 * * *',
  $$ SELECT net.http_post(
    url := current_setting('app.supabase_url') || '/functions/v1/generate-monthly-billing',
