...
// 3. Emojis mapping for ProfileScreen
...
// 4. Replace UI InfoRow
content = content.replace(
  /const InfoRow = \(\{ label, value, icon \}\) => \([\s\S]*?<\/View>\s*\);/,
...
content = content.replace(
  /const EditableInfoRow = \(\{ label, value, onChangeText, icon, placeholder, keyboardType = 'default' \}\) => \([\s\S]*?<\/View>\s*\);/,
...
// 7. Styles
content = content.replace(/infoEmoji:\s*\{[^}]*\},\s*/, "infoIcon: { width: 24, textAlign: 'center', marginRight: SPACING[2] },\n  ");
...
console.log('ProfileScreen.jsx restored and emojis replaced');
